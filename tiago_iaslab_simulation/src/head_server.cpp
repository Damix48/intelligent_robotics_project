...
void HeadServer::getObjects() {
  ...
  for (const auto& detection : msg->detections) {
    ...
    tag.pose = buffer.transform(detection.pose, baseFrame, ros::Duration(2));
...
control_msgs::FollowJointTrajectoryGoal HeadServer::generateHeadTrajectoryGoal(float pitch, float yaw) {
  ...
  headGoal.trajectory.points[0].velocities[0] = 0.5;
  headGoal.trajectory.points[0].velocities[1] = 0.5;
